...
//  ── 2. Online counter ────────────────────────────────────
//
//  HornHub.online.onChange(count => {
//    document.getElementById("online").textContent = count + " online";
//  });
//
//
//  ── 3. Views (video page / player open) ──────────────────
//
//  const { count } = await HornHub.views.trackView(videoId);
//  viewsEl.textContent = count + " views";
//
//
//  ── 4. Likes ─────────────────────────────────────────────
//
//  // On render — restore heart state (no network):
//  likeBtn.classList.toggle("liked", HornHub.likes.hasLiked(videoId));
//
//  // On heart click:
//  likeBtn.addEventListener("click", async () => {
//    const { count, already_liked } = await HornHub.likes.add(videoId);
//    likeCountEl.textContent = count;
//    if (!already_liked) likeBtn.classList.add("liked");
//    else alert("Already liked!");  // optional
//  });
//
//
//  ── 5. Comments ──────────────────────────────────────────
//
//  // Load on video open:
//  const { comments, has_more } = await HornHub.comments.load(videoId);
//  // Load next page:
//  const next = await HornHub.comments.load(videoId, 2);
//
//  // Post (no login — just a nickname):
//  const result = await HornHub.comments.post(videoId, "Ali", "Nice video!");
//  if (result) prependToList(result.comment);
//
//  // Delete own comment (button only shown if isOwn):
//  if (HornHub.comments.isOwn(comment)) {
//    deleteBtn.style.display = "block";
//    deleteBtn.onclick = async () => {
//      const ok = await HornHub.comments.delete(videoId, comment.id);
//      if (ok) commentEl.remove();
//    };
//  }
//
//
...
//    const count = await HornHub.shares.add(videoId);
//    shareCountEl.textContent = count;
//    navigator.share?.({ title: video.title, url: location.href });
//  });
//
//
//  ── 7. Download button ───────────────────────────────────
...
//    const count = await HornHub.downloads.add(videoId);
...
//  ── 8. Auth (100% optional) ──────────────────────────────
//
//  // Check if logged in (to show/hide login button):
//  const user = HornHub.auth.getUser();   // null = guest
//  if (user) showLogoutBtn(user.name);
//  else      showLoginBtn();
//
//  // Signup:
//  const res = await HornHub.auth.signup(email, password, name);
//  if (res.success) location.reload();
//  else alert(res.error);
//
//  // Login:
//  const res = await HornHub.auth.login(email, password);
//  if (res.success) location.reload();
//  else alert(res.error);
//
//  // Logout (back to guest — everything still works):
//  HornHub.auth.logout();
//  location.reload();
//
// ════════════════════════════════════════════════════════════
